...
AGENTS.md. **That record is the owner; this file does
...
1. **Reuse or extend before you create** — golden rule 2
 ...
 Search `src/components/ui` and
 `src/components source path`. Three outcomes, and only the third is a new component:
 - It exists → use it.
...
2. **Read the `docs/design/README.md` §5 spec.** If the component has no §5 entry — `Badge` and `rating-inline`
...
3. **Write the component.** `src/components/ui<name>/<name>.tsx`, `cva` table co-located,
...
7. **Export it** from `src/components/ui/index.ts`. It is the only public surface; a component missing
 from the barrel is unreachable (AGENTS.md).
8. **`bun run validate`, and `bun run test:e2e` because this is a component.** CI runs neither the stories nor the quality mode by default (AGENTS.md), and those are the two that render the component in a browser at all. `component-has-stories` and `tokens-resolve` both have opinions about what
 you just wrote.

## 0. Which package does it go in?

Two packages, split by what "correct" means for each (AGENTS.md):

| | `src/components/ui` (`src/components/ui`) | `src/components` (`src/components`) |
|---|---|---|
...
| May import `@base-ui/react` | **Yes, only here** | No — compose `src/components/ui` instead |
...
|---|---|---|
| `src/components/ui` | `src/components/<name>/` | `src/components/dialog/dialog.tsx` |
| `src/components` | `src/<feature>/<name>/` | `src/shell/top-bar/top-bar.tsx` |

`src/components` features are `shell`, `auth`, `wizard`, `results`, `sharing` — the brief's §2.x
...
and `<name>.quality.test.tsx` — in both packages. `src/components` composites additionally shipped a
`<name>.pixel.test.tsx` until August 2026; that layer is retired (docs/engineering/testing.md).
...
AGENTS.md records it, and
`component-has-stories.test.mjs` pins it.

`utils.ts` and `styles/` stay at `src/` root — both are matched by exact path by the guard
and the build.

**The test: would a designer recognise it by name from the design file?** Yes → composite. If it is a generic control that happens to wear this project's tokens → primitive. Genuinely unsure → `src/components/ui`, because promoting a primitive later is easier than splitting a composite apart.

**A composite is a pure function of its props and the message catalogue.** No `next/*`, no `src/lib/api.ts`, no `src/lib/api.ts`, no `the backend (new-api)`, no `fetch` — `bun run validate` fails the build on any of them. The pressure to break this is gradual and always sounds reasonable ("it just needs the current user"); when a composite needs data, a caller in `src/app` fetches it and passes it down. Do not relax the guard; move the component.

**Copy comes from `no i18n catalogue yet`**, not from props and not from the source. A composite's strings are fixed by the design, so a caller wanting different words wants a different component. Strings a caller passes into a slot are that caller's to translate.
...
Per-component visual specs are `docs/design/README.md` §5. Check there before
improvising.
...
Tailwind CSS v4 + src/app/globals.css decision owns this rule. What it does not carry is the vocabulary, which is Base UI's and differs
...
option and the chosen option need distinct treatment — `the component spec (openspec/changes/<id>/specs/)` §2 already requires this.
...
`the component spec (openspec/changes/<id>/specs/)` §1. Reference them; do not invent a number.
...
whether the two states belong to two widgets (AGENTS.md);
...
2.2 AA target `the component spec (openspec/changes/<id>/specs/)` §7 sets. It is invisible in the
...
`docs/design/README.md` §5.4 covers the panel's appearance and the 44×44 close button; the focus trap, Escape and
focus restore are Base UI's and are already specified in `the component spec (openspec/changes/<id>/specs/)` §3. **Do not reimplement
any of them.**

## 9. Forms use `Field`

`Field` wires the label, description and error to the control for you — which is exactly the
`aria-describedby` / `aria-invalid` association `the component spec (openspec/changes/<id>/specs/)` §6 and `docs/design/README.md` §5.2 already
...
<Field.Root name="email" validationMode="onBlur">
 ...
 <Field.Description>We only use this to send the results.</Field.Description>
...
- **`validationMode="onBlur"`** is what `the component spec (openspec/changes/<id>/specs/)` §6 already asks for — validate on blur, not
