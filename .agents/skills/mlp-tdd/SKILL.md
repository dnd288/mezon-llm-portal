---
name: tdd
description: "Test-first development for project tasks, and which of the four test modes applies to which claim. Load this when implementing any logic, fixing any bug, or changing any behaviour — a component, a service, a route, a migration — when a task from openspec/changes/<id>/tasks.md says a scenario needs coverage, or when asked what a test should look like for a given layer. Owns the RED→GREEN cycle, the <name>.<kind>.tsx naming that five globs key on, why a story is a fixture and not a test, and why geometry claims belong in Chromium, not jsdom. Routes to docs/engineering/testing.md and AGENTS.md for the strategy."
---
...
stories as fixtures, pixel baselines on composites only — is AGENTS.md
and docs/engineering/testing.md. This skill owns the discipline and the mode mapping.
...
standing: two comparative ones, `pixel` and `visual`, are retired (AGENTS.md),
...
| The layout matches the design source frame | `visual` (the design-truth gate) | `pixel` only proves *unchanged*; the visual layer compares against the tracked design source snapshot (AGENTS.md) |
...
| A session is revocable — a deleted row ends it | `api` unit against the store, **and** `mlp-e2e` | the only test that distinguishes a database session from a JWT wearing its name (AGENTS.md) |
...
| A mutation invalidates what it changed | `bun run test:e2e` | a unit test sees the action return, not the next render |
| A wizard step survives a reload or a shared link | `bun run test:e2e`, navigating **straight to the URL** | if it is not in the URL there is nothing to navigate to — which is the finding |
...
| The docs describe the new behaviour correctly | manual: `git diff` against `docs/`, `README.md`, `CONTEXT.md` | no automated tool reads a doc for semantic accuracy; the reviewer confirms the doc's terms match the code's terms |
...
The **visual layer is the design-truth gate** (AGENTS.md): it compares the rendered DOM
...
`manual check against docs/design/README.md` only when the design actually moved.
...
- [ ] `bun run validate` is green — it is typecheck + lint and needs no browser
- [ ] **If the change alters what a component looks like or how a screen behaves**, the deep layers were run too: `bun run test:e2e` and the e2e slice it touches (see `e2e/README.md` or `docs/engineering/testing.md` for the slice guidance). The whole suite is CI's; locally, the slice must cover the specific change.
- [ ] **Docs in sync:** if this change ships a behaviour that any doc in `docs/`, `README.md`, or `CONTEXT.md` describes in old terms, the doc was updated or explicitly marked as not needing change
- [ ] **E2E evidence:** for each FR this change implements, the feature file in `e2e/features/*.feature` was added or updated and the slice has been run with a recorded result in `verification.md` (or an explicit docs-only/copy-only N/A)