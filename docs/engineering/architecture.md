...
| Client islands | `src/components/*` (dialog, delete button, mobile nav) | `'use client'`; interaction only — receive data via props, mutate via `/api/portal/*` fetch, never import `src/lib/api.ts` |
